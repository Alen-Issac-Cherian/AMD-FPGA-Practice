...
//	TimingPtr_0 = XVidC_GetTimingInfo(resId_0);
	...
	//xil_printf("CCM, Image Enhancer and Sensor demosaic started in hardware\r\n");

	//Configure HLS ISP pipe
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0010, 0x190);
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0018, 0x190);
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0030, 0x80);
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0038, 0x80);
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0020, 0x80);
	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0054, 0x83);
	for(int i = 0; i < 256; i++)
	{
		Xil_Out8(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0800 +i, i);
	}
	for(int i = 0; i < 256; i++)
	{
		Xil_Out8(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0900 +i, i);
	}
	for(int i = 0; i < 256; i++)
	{
		Xil_Out8(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0A00 +i, i);
	}

	Xil_Out32(XPAR_ISPPIPELINE_ACCEL_0_S_AXI_CTRL_BASEADDR + 0x0000, 0x81);

	xil_printf("HLS ISP pipe Setup Complete!\r\n");
